...
export default function BookNow() {
  ...
  const [formData, setFormData] = useState<{
    ...
    userType: "",
    grade: "",
    selectedSubjects: [], // This will now be treated as a string array
    studentInfo: {
      firstName: "",
      lastName: "",
      reason: "",
  ...
  const handleLessonInputChange = (e: { name: string; value: string }) => {
    const { name, value } = e
  ...
  const handleLastInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target
  ...
  const handleGradeChange = (grade: string) => {
    setFormData((prevData) => {
      const updatedData = prevData.grade !== grade ? { ...prevData, grade, selectedSubjects: [] } : prevData
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prevFormData) => {
      ...
      const currentAvailability =
        prevFormData.lessonDetails.availability[day as keyof typeof prevFormData.lessonDetails.availability] || []
      ...
      return {
        ...
        lessonDetails: {
          ...
          availability: {
            ...
            [day]: updatedAvailability, // Update the specific day's availability
          },
        },
      }
    })
        ...
        return (
          <div className="step-content">
            ...
            <div className="mb-4">
              <label className="font-semibold mb-1 block">
                {formData.userType === "Student"
                  ...
                  : "Why are you seeking a tutor for your child?"}
              </label>
              <select
                name="reason"
            ...
            <div className="mb-4">
              <label className="font-semibold mb-1 block">
                {formData.userType === "Student"
                  ? "How would you describe your current performance in school?"
                  : "How would you describe your child's current performance in school?"}
              </label>
              <select
                name="performance"
          ...
          <div className="step-content4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-2">
              ...
              <div className="flex-1">
                ...
                <select
                  name="duration"
                  value={formData.lessonDetails.duration.endsWith("hours") ? "custom" : formData.lessonDetails.duration} // Keeps "custom" selected when custom value is present
                  onChange={(e) => {
                    ...
                    if (value === "custom") {
                      ...
                      handleLessonInputChange({ name: "duration", value })
                    }
                  }}
                  className="p-2 border rounded w-full"
                >
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        onChange={(e) => {
                          const value = e.target.value
                          const durationValue = Number.parseInt(value, 10)
                ...
                <select
                  ...
                  value={
                    formData.lessonDetails.frequency.endsWith("times a week")
                      ? "custom"
                      : formData.lessonDetails.frequency
                  } // Keeps "custom" selected when custom value is present
                  onChange={(e) => {
                    const value = e.target.value
                    if (value === "custom") {
                      handleLessonInputChange({
                      ...
                      handleLessonInputChange({ name: "frequency", value })
                    }
                  }}
                  className="p-2 border rounded w-full"
                >
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        onChange={(e) => {
                          const value = e.target.value
                          const frequencyValue = Number.parseInt(value, 10)
                ...
                <tbody>
                  {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"].map((day) => (
                    <tr key={day}>
                      ...
                      {["Early Morning", "Morning", "Early Afternoon", "Late Afternoon", "Evening", "Any"].map(
                        (time) => {
                          type Weekday =
                            | "Monday"
                            | "Tuesday"
                          ...
                          const isDisabled =
                            (formData.lessonDetails.availability[day as Weekday] as string[])?.includes("Any") ||
                            (formData.lessonDetails.availability[day as Weekday] as string[])?.includes("Unavailable")

                          const isChecked = (formData.lessonDetails.availability[day as Weekday] as string[])?.includes(
                            time,
                          )
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <p className="mb-2">
                  ...
                  {formData.selectedSubjects.length > 0 ? formData.selectedSubjects.join(", ") : "No subjects selected"}
                </p>
                <p className="mb-2">
                ...
                <p className="mb-2">
                  ...
                  {formData.lessonDetails.duration || "Not selected"}
                </p>
                <p className="mb-2">
                  <span className="font-semibold">Lesson Frequency:</span>{" "}
                  {formData.lessonDetails.frequency || "Not selected"}
                </p>
  ...
  const renderSubjects = (grade: string) => {
    const handleSubjectChange = (subject: string) => {
      setFormData((prevData) => {
        const updatedSubjects = prevData.selectedSubjects.includes(subject)
          ? prevData.selectedSubjects.filter((item) => item !== subject)
      ...
      return (
        <div>
          {[
            {
              name: "English",
              options: ["English Standard", "English Advanced", "English Extension 1", "English Extension 2", "EAL/D"],
            },
            {
              name: "Maths",
              options: [
            ...
            {
              name: "Humanities",
              options: ["Ancient History", "Modern History", "History Extension", "Geography", "Society and Culture"],
            },
            {
              name: "Other",
              options: ["Languages", "Studies of Religion", "Software Development", "IT", "Visual Arts", "Music"],
            },
          ].map((subject) => (
  ...
  return (
    <div className="mt-16 flex flex-col items-center w-full">
      ...
      <div className="text-center py-8 px-4">
        <h1 className="text-2xl md:text-4xl font-bold mb-4">Start Your Journey with JDN Tuition Today</h1>
        <p className="text-gray-700 text-lg">Our streamlined process gets you matched and learning in no time:</p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8 md:gap-6 px-4">
        ...
        <div className="mt-4 border-2 border-[#17A4A5] rounded-lg p-6 shadow-lg flex flex-col items-center max-w-[250px] mx-auto">
          ...
          <h2 className="text-lg font-semibold mb-4 text-center">Tell Us Your Story</h2>
          <p className="text-gray-700 text-sm text-center leading-5">
            Start by filling our booking form to share your details and tutoring needs. There&apos;s no commitment at
            this stage — it&apos;s just to help us understand how we can assist you best.
          </p>
        </div>
        ...
        <div className="mt-4 border-2 border-[#17A4A5] rounded-lg p-6 shadow-lg flex flex-col items-center max-w-[250px] mx-auto">
          ...
          <h2 className="text-lg font-semibold mb-4 text-center">15-Minute Free Consultation</h2>
          <p className="text-gray-700 text-sm text-center leading-5">
            Meet your tutor for a quick, no-obligation consultation. This is your opportunity to ask questions, discuss
            your goals, and make sure that it&apos;s the right fit.
          </p>
        </div>
                  ...
                  disabled={
                    ...
                    (step === 5 &&
                      (!formData.parentDetails.email ||
                        !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.parentDetails.email))) ||
